	...
	return {
		...
		controller:  ['$scope', 'orientation', 'mapLoader', function($scope, orientation, mapLoader) {
			$scope.$on('sceneReady', function(e, s) {
				...
				var liveSources = {};
				var addVideo = function(event) {
					var videoSocketID = event._event,
						videoSource = $scope.sources[videoSocketID],
					...
					liveSources[videoSocketID] = videoSource;
				};
				...
				$scope.$watch('localSource', function(localSource) {
					if (localSource) {
						facerace.video(localSource.socketID);
					}
				});
